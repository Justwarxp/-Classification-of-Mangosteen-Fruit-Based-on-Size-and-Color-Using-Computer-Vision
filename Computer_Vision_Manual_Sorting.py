...
ukuran_kecil = 4  # < 4cm
ukuran_sedang = [4, 8]  # (rentang nilai 4 sampai 8 cm)
ukuran_besar = 8  # > 8cm
...
while True:
    ...
    # untuk menangkap masukan keyboard, angka 1 untuk menangkap gambar dan memulai menyimpan dan menghitung hasil hitung ke server IOT
    if msvcrt.kbhit():
        print("Letakan Buah lalu Tekan 1 untuk mulai : ")
                ...
                try:
                    ...
                    if label == "manggis_hijau":
                        ...
                        if ukuran_buah <= ukuran_kecil:
                            #print("ukuran kecil manggis_hijau")
                            # cv2.putText(frame, "Kecil", (int(
                            #     box[0]) + 10, int(box[1]) + 100), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (255, 255, 255), 2)
                            # if deteksi == "1":
                            #     status = 1
                            #     # jumlah_manggis_hijau[0] += 1
